...
def verified_bundle_times(archive_root: Path) -> list[datetime]:
    ...
    """
    collections = archive_root / "collections"
...
def cadence_health(config, bundle_times: list[datetime], now_utc: datetime) -> dict:
    ...
    if now < start:
        ...
        status, healthy = "window_complete", True
...
def main() -> None:
    ...
    report = cadence_health(config, verified_bundle_times(args.archive_root), now)
